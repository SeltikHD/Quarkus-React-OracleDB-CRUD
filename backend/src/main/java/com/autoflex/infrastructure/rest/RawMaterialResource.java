...
public class RawMaterialResource {
	...
	public RawMaterialResource(
			RawMaterialUseCase rawMaterialUseCase, RawMaterialRestMapper mapper) {
	...
	public Response createRawMaterial(@Valid RawMaterialRequest request) {
		...
		return Response.created(
				URI.create("/api/v1/raw-materials/" + created.getId().value()))
				.entity(response)
				.build();
	}
	...
	public RawMaterialResponse adjustStock(
		...
		RawMaterial updated = rawMaterialUseCase.adjustStock(
				RawMaterialId.of(id), request.getQuantity());
		return mapper.toResponse(updated);
	}
