 ...
 *
 * <p><b>HEXAGONAL ARCHITECTURE:</b> This is an INPUT ADAPTER (also called "driving adapter" or
 * "primary adapter"). It receives HTTP requests and translates them to domain use case calls.
 *
 * <p><b>RESPONSIBILITIES:</b>
 *
 * <ul>
 *   <li>HTTP request/response handling
 *   <li>DTO to domain command mapping
 *   <li>Domain entity to DTO response mapping
 *   <li>OpenAPI documentation
 * </ul>
 */
...
public class ProductResource {
  ...
  public List<BillOfMaterialItemResponse> getProductMaterials(@PathParam("id") Long id) {
    Product product = productUseCase.getProductById(ProductId.of(id));
    return product.getMaterials().stream()
        .map(
            bom ->
                new BillOfMaterialItemResponse(bom.rawMaterialId().value(), bom.quantityRequired()))
        .toList();
  }
  ...
  public ProductResponse addMaterial(
      @PathParam("id") Long id, @Valid BillOfMaterialItemRequest request) {
    ProductUseCase.AddMaterialCommand command =
        new ProductUseCase.AddMaterialCommand(
            request.getRawMaterialId(), request.getQuantityRequired());
  ...
  public ProductResponse updateMaterialQuantity(
    ...
    Product updated =
        productUseCase.updateMaterialQuantity(
  ...
  public Response removeMaterial(
      @PathParam("id") Long id, @PathParam("rawMaterialId") Long rawMaterialId) {
