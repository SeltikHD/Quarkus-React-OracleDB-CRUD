...
import com.autoflex.infrastructure.rest.dto.BillOfMaterialItemRequest;
...
import com.autoflex.infrastructure.rest.dto.UpdateMaterialQuantityRequest;
 ...
 * This is an INPUT ADAPTER (also called "driving adapter" or "primary
 * adapter").
...
public class ProductResource {
    ...
    @APIResponse(responseCode = "200", description = "Products retrieved successfully", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(type = SchemaType.ARRAY, implementation = ProductResponse.class)))
    ...
    @APIResponse(responseCode = "200", description = "Stock adjusted successfully", content = @Content(schema = @Schema(implementation = ProductResponse.class)))
    @APIResponse(responseCode = "400", description = "Invalid quantity or insufficient stock")
    @APIResponse(responseCode = "404", description = "Product not found")
    public ProductResponse adjustStock(
            @Parameter(description = "Product ID", required = true) @PathParam("id") Long id,
            @Parameter(description = "Quantity to add (positive) or remove (negative)", required = true) @QueryParam("delta") int delta) {
    ...
    public Response deactivateProduct(
            @Parameter(description = "Product ID", required = true) @PathParam("id") Long id,
            @Parameter(description = "Permanently delete instead of soft-delete") @QueryParam("permanent") @DefaultValue("false") boolean permanent) {
    ...
    @GET
    @Path("/{id}/materials")
    @Operation(summary = "Get product's bill of materials")
    public List<BillOfMaterialItemResponse> getProductMaterials(
    ...
    @POST
    @Path("/{id}/materials")
    @Operation(summary = "Add material to product BOM")
    public ProductResponse addMaterial(
            @PathParam("id") Long id,
            @Valid BillOfMaterialItemRequest request) {
        ProductUseCase.AddMaterialCommand command = new ProductUseCase.AddMaterialCommand(
                request.getRawMaterialId(),
                request.getQuantityRequired());
        Product updated = productUseCase.addMaterialToProduct(ProductId.of(id), command);
        return mapper.toResponse(updated);
    }

    @PUT
    @Path("/{id}/materials/{rawMaterialId}")
    @Operation(summary = "Update material quantity in BOM")
    public ProductResponse updateMaterialQuantity(
            @PathParam("id") Long id,
            @PathParam("rawMaterialId") Long rawMaterialId,
            @Valid UpdateMaterialQuantityRequest request) {
        Product updated = productUseCase.updateMaterialQuantity(
                ProductId.of(id), rawMaterialId, request.getQuantityRequired());
        return mapper.toResponse(updated);
    }

    @DELETE
    @Path("/{id}/materials/{rawMaterialId}")
    @Operation(summary = "Remove material from product BOM")
    public Response removeMaterial(
            @PathParam("id") Long id,
            @PathParam("rawMaterialId") Long rawMaterialId) {
        productUseCase.removeMaterialFromProduct(ProductId.of(id), rawMaterialId);
        return Response.noContent().build();
    }
}
