...
public class ProductRequest {
    ...
    @Schema(description = "Product description", example = "High-quality electronic component for industrial use")
    private String description;

    @NotBlank(message = "SKU is required")
    @Pattern(regexp = "^[A-Za-z0-9-]+$", message = "SKU can only contain letters, numbers, and hyphens")
    @Size(max = 50, message = "SKU cannot exceed 50 characters")
    @Schema(description = "Stock Keeping Unit (unique identifier)", example = "COMP-XYZ-001", required = true)
    private String sku;
